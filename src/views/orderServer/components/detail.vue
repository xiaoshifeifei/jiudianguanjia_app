    ...
    <div class="content-1">
      ...
      <div class="contentshop">
        ...
        <div class="comitTime">
          ...
          <div class="comitTime-2" v-if="statusParams === '2' || statusParams === '3'">{{listParams.ReceiveDate}}</div>
          ...
          <div class="comitTime-2" v-if="statusParams === '3'">{{listParams.CancelDate}}</div>
          ...
          <div class="comitTime-2" v-if="statusParams === '2' || statusParams === '3' || statusParams === '4' || statusParams === '5'">{{listParams.OperatorName}}</div>
...
import { 
  ...
  SupermarketReceive,
  serveOrder,
  serveOrdeReceive  } from '@/api/supermarket.js'
export default {
  data() {
    return {
      textInfo: '',
  ...
  methods: {
    ...
    validator(val) {
     ...
     async getList() {
      ...
      const getConven = await getServeOrder({
    ...
    async commitOrderOk(params) {
      this.orderID = this.$route.query.OrderID
      if (params == 1) {
        const getConven = await serveOrdeReceive({
          OrderID: this.orderID,
        ...
        const getConven = await serveOrdeorPerate({
    ...
    async commitOrders(params) {
      ...
      const getConven = await serveOrder({
        OrderID: this.orderID,
        CancelOrderStatus: -1,
