  ...
  <div class="mains">
    ...
    <div class="content-1">
      ...
      <div class="contentshop">
        <div class="content-1">不住退房</div>
        <div class="content-2">
          <div class="content-3">
            <div class="content-4">续住时间: {{listParams.CheckOutDate}}</div>
          </div>
        </div>
        <div class="order-1">
          <div class="order-2">
...
import { 
  ...
  getCheckCancle,
  getCheckReceive,
  getCheckComplete  } from '@/api/supermarket.js'
export default {
  data() {
    return {
      textInfo: '',
  ...
  methods: {
    ...
    validator(val) {
     ...
     async getList() {
      ...
      const getConven = await getCheckOutShow({
    ...
    async commitOrderOk(params) {
      this.orderID = this.$route.query.OrderID
      if (params == 1) {
        const getConven = await getCheckReceive({
          OrderID: this.orderID,
        ...
        const getConven = await getCheckComplete({
    ...
    async commitOrders(params) {
      ...
      const getConven = await getCheckCancle({
        OrderID: this.orderID,
        CancelOrderStatus: -1,
