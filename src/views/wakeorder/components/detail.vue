<template>
  <div class="mains">
    <!-- 内容区域 -->
    <div class="content-1" v-for="(item,idnex) in list" :key="idnex">
      <!-- 头部 -->
      <div class="content-2">
        ...
        <div class="content-4">
          <div class="item-content1">{{item.UserName}}<span v-if="item.UserName && item.UserTel">/</span>{{item.Tel}}</div>
        </div>
      </div>
      <!-- 商品区域 -->
      <div class="contentshop">
        <div class="content-1">预约叫醒时间:{{item.WakeDate}}</div>
        <div class="order-1">
          <div class="order-2">
            <span>订单编号: </span>
        ...
        <div class="comitTime">
          ...
          <div class="comitTime-2">{{item.CreateDate}}</div>
          <div class="comitTime-1" v-if="statusParams === '2' || statusParams === '4'">接单时间</div>
          <div class="comitTime-2" v-if="statusParams === '2' || statusParams === '4'">{{item.WakeDate}}</div>
          <div class="comitTime-1" v-if="statusParams === '4'">完成时间</div>
          <div class="comitTime-2" v-if="statusParams === '4'">{{item.CompleteDate}}</div>
          <div class="comitTime-1" v-if="statusParams === '3'">取消时间</div>
          <div class="comitTime-2" v-if="statusParams === '3'">{{item.WakeDate}}</div>
          <div class="comitTime-1" v-if="statusParams === '2' || statusParams === '3' || statusParams === '4'">操作人</div>
          <div class="comitTime-2" v-if="statusParams === '2' || statusParams === '3' || statusParams === '4'">{{item.OperatorName}}</div>
  ...
  methods: {
    ...
    validator(val) {
     ...
     async getList() {
      ...
      const getConven = await getlocalShow({
    ...
    async commitOrders(params) {
      ...
      const getConven = await wackOrder({
        OrderID: this.orderID,
        CancelOrderStatus: -1,
