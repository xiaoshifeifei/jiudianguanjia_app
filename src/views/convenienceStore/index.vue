  ...
  <div class="mains">
    ...
    <div class="tabs">
      ...
      <div class="div-2" @click="clickDps">
        <span class="span-1" :class="{ actions : isActionsDps}">待配送</span>
    ...
    <div class="content-1"  v-for="(item,index) in list" :key="index">
      ...
      <div class="content-2" @click.stop="clickDetails(item.OrderID)" >
        <div class="content-3">
          ...
          <div class="item-content2" v-if="myStatus === '2'">待配送</div>
      ...
      <div class="contentshop">
        ...
        <div class="foot-btn">
          ...
          <!-- 退款操作未完成 -->
          <div class="foot-le" @click="isSorrys(item.OrderID,'2')" v-if="myStatus === '2' && item.IsAllowRefund == 1">
            退款
          </div>
          <!-- 已完成状态 -->
          <div class="foot-ri" @click="isSorrys(item.OrderID,'3')" v-if="myStatus === '3' && item.IsAllowRefund == 1">
            退款
          </div>
...
import { 
  ...
  SupermarketComplete,
  SupermarketCancel,
  SupeRefund } from '@/api/supermarket.js'
export default {
  components: {
    searchClick
  },
  ...
  methods: {
    async getList(params) {
      ...
      const getConven = await getSupermarket({
        Page: 1,
        Limit: 100,
        m_status: params,
        Search: ''
    ...
    async commitOrders(params) {
      ...
      if (params == 2) {
        const getConven = await SupeRefund({
          OrderID: this.OrderIDParams,
          RefundOrderStatus: 1,
          RefundRemark: this.textInfo
        })
        console.log("getConven",getConven);
        if (getConven.code == 0) {
          this.$toast("订单已退款")
