  ...
  <div class="mains">
    ...
    <div class="content-1">
      ...
      <div class="contentshop">
        <div class="content-1">预约叫醒时间:{{listParams.BookDate}}</div>
        <div class="order-1">
          <div class="order-2">
            <span>订单编号: </span>
        ...
        <div class="comitTime">
          ...
          <div class="comitTime-2" v-if="statusParams === '3'">{{listParams.BookDate}}</div>
...
import { 
  ...
  cleanOrder,
  cleanOrdeReceive,
  cleanOrdeorPerate  } from '@/api/supermarket.js'
export default {
  data() {
    return {
      textInfo: '',
  ...
  methods: {
    ...
    validator(val) {
     ...
     async getList() {
      ...
      const getConven = await getClean({
      ...
      if(getConven.code == 0) {
        ...
        this.listParams.BookDate = res.BookDate  //预约叫醒时间
        this.listParams.ReceiveDate = res.ReceiveDate  //接单时间
        this.listParams.SendDate = res.SendDate  //发货时间
        this.listParams.CompleteDate = res.UpdateDate  //完成时间
    ...
    async commitOrderOk(params) {
      this.orderID = this.$route.query.OrderID
      if (params == 1) {
        const getConven = await cleanOrdeReceive({
          OrderID: this.orderID,
        ...
        const getConven = await cleanOrdeorPerate({
          OrderID: this.orderID,
    ...
    async commitOrders(params) {
      ...
      const getConven = await cleanOrder({
        OrderID: this.orderID,
        CancelOrderStatus: -1,
