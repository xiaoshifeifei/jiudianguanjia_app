<template>
  <div class="mains">
    ...
    <div class="content-1" v-for="(item, index) in list" :key="index">
      <!-- 头部 -->
      <div class="content-2" @click.stop="clickDetails(item.InvoiceOrderID)">
        <div class="content-3">
          <div class="item-content1">
            房号:<span>{{ item.RoomNumber }}</span>
          </div>
        ...
        <div class="content-4">
          <div class="item-content1">
            {{ item.UserName }}<span v-if="item.UserName && item.Tel">/</span>{{ item.Tel }}
          </div>
      ...
      <div class="contentshop">
        ...
        <div class="content-2">
          <div class="content-3">
            <div class="content-4">开票类型</div>
            <div class="content-5">{{item.InvoiceTypeName}}</div>
          </div>
          <div class="content-3">
            <div class="content-4">抬头类型</div>
            <div class="content-5">{{item.Title}}</div>
          </div>
          <div class="content-3"  v-if="isActions">
            <div class="content-4">真实姓名</div>
            <div class="content-5">{{item.MainBodyTypeName}}</div>
          </div>
          
          <div class="content-3"  v-if="isActions">
            <div class="content-4">电话号码</div>
            <div class="content-5">{{item.Tel}}</div>
          </div>
          <!-- isActionsDps -->
          <div class="content-3" v-if="isActionsDps">
            <div class="content-4">开户银行</div>
            <div class="content-5">{{item.Bank}}</div>
          </div>
          <div class="content-3" v-if="isActionsDps">
            <div class="content-4">银行账号</div>
            <div class="content-5">{{item.BankAccountNumber}}</div>
          </div>
          <div class="content-3">
            <div class="content-4">发票类型</div>
            <div class="content-5">{{item.ElectronicPaperTypeName}}</div>
          </div>
          <div class="content-3" v-if="isActionsDps">
            <div class="content-4">单位电话</div>
            <div class="content-5">{{item.CompanyTel}}</div>
          </div>
          <div class="content-3" v-if="isActionsDps">
            <div class="content-4">单位地址</div>
            <div class="content-5">{{item.CompanyAddress}}</div>
          </div>
          <div class="content-3" v-if="isActionsDps">
            <div class="content-4">联系电话</div>
            <div class="content-5">{{item.Tel}}</div>
          </div>
        </div>
        ...
        <div class="order-1">
          <div class="order-2">
            <span>订单编号: </span>
            <span>{{ item.InvoiceOrderNumber }}</span>
          </div>
          <div class="order-3">
            <span>备注: </span>
        ...
        <div class="foot-btn">
          <!-- 待结单状态 item.OrderID-->
          <div class="foot-ri" @click="getOrders(item.InvoiceOrderID)" v-if="myStatus === '1'">
            接单
          </div>
          <div class="foot-le" @click="isSorry(item.InvoiceOrderID)" v-if="myStatus === '1'">
            拒绝
          </div>
          <!-- 带配送状态 -->
          <div class="foot-ri" @click="getComplete(item.InvoiceOrderID)" v-if="myStatus === '2'">
            完成
          </div>
          <div class="foot-le" @click="isSorrySe(item.InvoiceOrderID)" v-if="myStatus === '2'">
            取消
          </div>
          <!-- 已完成状态 -->
...
export default {
  ...
  methods: {
    async getList(params) {
      const getConven = await getInvoice({
        Page: 1,
        Limit: 100,
        mStatus: params,
        Search: '',
    ...
    async commitOrderOk(params) {
      if (params == 1) {
        const getConven = await cleanOrdeInvoice({
          InvoiceOrderID: this.orderIDParams,
        ...
        const getConven = await cleanoPerateInvoice({
          OperateInvoiceStatus: 2,
          InvoiceOrderIDs: [this.orderIDParams],
        })
        console.log('getConven', getConven)
        if (getConven.code == 0) {
          this.isPopups = false
