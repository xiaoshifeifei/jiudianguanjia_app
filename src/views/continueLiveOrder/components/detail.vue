  ...
  <div class="mains">
    ...
    <div class="content-1">
      ...
      <div class="content-2">
        ...
        <div class="content-4">
          <div class="item-content1">{{listParams.UserName}}<span v-if="listParams.UserName && listParams.UserTel">/</span>{{listParams.UserTel}}</div>
        </div>
      </div>
      <!-- 商品区域 -->
      <div class="contentshop">
        <div class="content-1">续住信息</div>
        <div class="content-2">
          <div class="content-3">
            <div class="content-4">续住时间: {{listParams.ContinueLiveDate}} 至 {{listParams.CheckOutDate}}</div>
          </div>
        </div>
        ...
        <div class="comitTime">
          ...
          <div class="comitTime-2" v-if="statusParams === '2' || statusParams === '4'">{{listParams.ReceiveDate}}</div>
          <div class="comitTime-1" v-if="statusParams === '4'">完成时间</div>
          <div class="comitTime-2" v-if="statusParams === '4'">{{listParams.CheckOutDate}}</div>
...
import { 
  ...
  getContinueCancel,
  getLiveReceive,
  getLiveOperate  } from '@/api/supermarket.js'
export default {
  data() {
    return {
      textInfo: '',
  ...
  methods: {
    ...
    validator(val) {
     ...
     async getList() {
      ...
      const getConven = await getLiveOrder({
      ...
      if(getConven.code == 0) {
        ...
        this.listParams.InvoiceOrderNumber = res.OrderSerialNo  //编号
        ...
        this.listParams.ContinueLiveDate = res.ContinueLiveDate  //入住时间
        this.listParams.CheckOutDate = res.CheckOutDate  //离开时间
        ...
        this.listParams.CheckOutDate = res.CheckOutDate  //完成时间
    ...
    async commitOrderOk(params) {
      this.orderID = this.$route.query.OrderID
      if (params == 1) {
        const getConven = await getLiveReceive({
          OrderID: this.orderID,
        ...
        const getConven = await getLiveOperate({
    ...
    async commitOrders(params) {
      ...
      const getConven = await getContinueCancel({
        OrderID: this.orderID,
        CancelOrderStatus: -1,
