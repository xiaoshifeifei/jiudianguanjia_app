  ...
  <div class="mains">
    ...
    <div class="tabs">
      ...
      <div class="div-2" @click="clickDps">
        <span class="span-1" :class="{ actions : isActionsDps}">待租赁</span>
        <span class="span-2" :class="{ actions1 : isActionsDps}">{{orderCounts.receive}}</span>
      </div>
      <div class="div-3" @click="clickYwc">
        <span class="span-1" :class="{ actions : isActionsYwc}">已租赁</span>
        <span class="span-2" :class="{ actions1 : isActionsYwc}">{{orderCounts.done}}</span>
      </div>
      <div class="div-4" @click="clickYqx">
        <span class="span-1" :class="{ actions : isActionsYqx}">已归还</span>
        <span class="span-2" :class="{ actions1 : isActionsYqx}">{{orderCounts.over}}</span>
      </div>
      <div class="div-5" @click="clickYtk">
        <span class="span-1" :class="{ actions : isActionsYtk}">已取消</span>
        <span class="span-2" :class="{ actions1 : isActionsYtk}">{{orderCounts.cancel}}</span>
      </div>
    </div>
    ...
    <div class="content-1"  v-for="(item,index) in list" :key="index">
      ...
      <div class="content-2" @click.stop="clickDetails(item.OrderID)" >
        <div class="content-3">
          ...
          <div class="item-content2" v-if="myStatus === '2'">待租赁</div>
          <div class="item-content2" v-if="myStatus === '3'">已租赁</div>
          <div class="item-content2" v-if="myStatus === '4'">已归还</div>
      ...
      <div class="contentshop">
        ...
        <div class="zInfo-1">
          ...
          <div class="zInfo-right">
            <div class="zInfo-right1">
              <span class="zInfo-span">{{item.GoodsName}}</span>
            </div>
            <div class="zInfo-right2">
              <span class="zInfo-right3">￥</span>
              <span class="zInfo-right4"><span>{{item.RentPrice}}</span>/天</span>
            </div>
          </div>
        </div>
        ...
        <div class="order-1">
          <div class="order-4">
            <span>租车日期: </span>
            <span>{{item.RentBeginDate}}</span>
          </div>
          <div class="order-5">
            <span>合计金额: </span>
            <span> <span class="order-7">￥</span><span class="order-6">{{item.RentTotalPrice}}</span></span>
          </div>
          <div class="order-2">
            <span>订单编号: </span>
...
import { 
  ...
  getRentReceive,
  getRentComplete,
  getRentLease } from '@/api/supermarket.js'
export default {
  components: {
    searchClick
  },
  ...
  methods: {
    async getList(params) {
      ...
      if(getConven.code == 0) {
        ...
        this.orderCounts.over = resData.over  //处理完成
        this.orderCounts.receive = resData.receive  //已结单
      }
    },
    ...
    async commitOrderOk(params) {
      if (params == 1) {
        const getConven = await getRentReceive({
        ...
        const getConven = await getRentLease({
          OrderID: this.orderIDParamss,
          LeaseOrderStatus: 1
        })
        if (getConven.code == 0) {
        ...
        const getConven = await getRentComplete({
          OrderID: this.orderIDParamss,
    ...
    async commitOrders(params) {
      ...
      if (params == 1) {
        const getConven = await getRentOrder({
